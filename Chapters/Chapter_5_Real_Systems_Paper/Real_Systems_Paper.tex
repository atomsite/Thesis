\chapter[Exploring Dust Growth in WR140]{\secondpapertitle{} \parencite{eatsonExploringDustGrowth2022}}
\label{ch:wr140}
...
\begin{abstract}
  ...
  \small{
    ...
    In extreme cases, approximately 30\% of the total mass loss rate of a system can be converted into dust \parencite{lauRevisitingImpactDust2020}.
...
Despite this dust-hostile environment, CWB systems containing a Wolf-Rayet carbon phase star (WC) have been observed producing copious quantities of dust (so-called WCd systems; see \textcite{Williams1995}).
...
The initial mechanism behind dust formation is not well understood, but significant quantities of amorphous carbon grains have been observed to form within the WCR of a number of CWB systems.
...
The integrators used were a 3\ts{rd} order accurate Runge-Kutta integrator (\texttt{rk3}), as well as a 4\ts{th} order accurate, 5-stage, 3 storage register strong stability preserving Runge-Kutta integrator (\texttt{ssprk5\_4}; \cite{ruuthHighOrderStrongStabilityPreservingRungeKutta2005}).
...
The lookup table was generated by mixing a series of cooling curves from a MEKAL calculation of elemental gasses \parencite{mewe1995update}.
These curves were combined based on the elemental abundances in the WC \parencite{sanderGalacticWCStars2012} and OB \parencite{andersAbundancesElementsMeteoritic1989} winds.
...
Dust grows through grain accretion using formulae described by \textcite{spitzerPhysicalProcessesInterstellar2008} where dust grains grow via low-velocity collisions with surrounding carbon atoms, causing them to accrete onto the surface of the dust grain.
...
Similarly to the gas/plasma emission model used, the emitted photons are not re-\note{absorbed} by the WCR medium, causing energy to be removed from the simulation.
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    \hline
  \end{tabular}
  \caption[Grain critical energy]{Grain critical energy, $E^*$, for a dust grain of $a$ in \si{\micro\metre} for electrons, $e^-$, as well as the elements considered for grain cooling. The values for carbon, oxygen and nitrogen are identical \parencite{dwek_infrared_1981}.}
...
We have previously simulated WCd systems in the form of a parameter space exploration, in order to discern which wind and orbital parameters are influential on the dust growth rates \parencite{eatsonExplorationDustGrain2022}.
...
\noindent
where $\theta\rms{WR}$ is the opening angle of the WR shock front ($\theta\rms{WR} \approx 2 \tan^{-1}(\eta^{1/3}) + \pi/9$, see \textcite{pittardCollidingStellarWinds2018}).
...
\begin{table}
  ...
  \begin{tabular}{lll}
    ...
    \hline
    $\text{M}_\text{WR}$ & $10.31 \pm 0.45 \, \si{\solarmass}$& \textcite{thomasOrbitStellarMasses2021} \\
    $\text{M}_\text{OB}$ & $29.27 \pm 1.14 \, \si{\solarmass}$ & \textcite{thomasOrbitStellarMasses2021} \\
    $P$ & $2895.00\pm0.29 \, \si{d}$ & \textcite{thomasOrbitStellarMasses2021} \\
    $e$ & $0.8993 \pm 0.0013$ & \textcite{thomasOrbitStellarMasses2021} \\
...
\begin{table}
  ...
  \hline
  \end{tabular}
  \caption[Abundances by mass used for OB and WR stars]{Abundances used for the OB and WR stars being simulated. Other elements are assumed to be trace when calculating dust emission \parencite{williamsSpectraWC9Stars2015,sanderGalacticWCStars2012,andersAbundancesElementsMeteoritic1989}}.
...
% \begin{figure}
%   \centering
%   \includegraphics{assets/wr140-dt.pdf}
...
This maximum dust production rate of \SI{1.24e-6}{\solarmass\per\year} is sensible, but somewhat prodigious.
...
This is consistent with $L^\prime$ photometry of WR140 \parencite{crowther_dust_2003} -- the photometry magnitude sharply increases over the first year -- and suggests our model can qualitatively assess dust production rates of episodic WCd systems.
...
These results are also in agreement with more contemporary research by \textcite{pollockCompetitiveXRayOptical2021} which notes a similar sharp increase in K-band flux and x-ray column density a short time after periastron passage.
...
Furthermore, \textcite{pollockCompetitiveXRayOptical2021} notes that x-ray cooling becomes less influential, and instead transitions to an optical cooling regime shortly after periastron passage, indicating a rapid rate of cooling in the immediate post-shock environment.
...
This is theorised to be due to the increased cooling in the WCR caused by slower winds, as well as through KH instabilities driven by a wind velocity shear (if the wind terminal velocities are significantly different, see \textcite{stevens_colliding_1992}).
...
We also note that in reality there could be significant change in the pre-shock velocity of the O wind due to the WCR encroaching into the acceleration region during periastron \parencite{sugawaraSuzakuMonitoringWolf2015}.
...
We would also like to thank P. A. Crowther for his work on the \footlink{Galactic Wolf-Rayet Catalogue}{http://pacrowther.staff.shef.ac.uk/WRcat}.
Finally, we would like to thank the academic referee for their extremely helpful review.

\section*{Data Availability}
The data underlying this article are available in the Research Data Leeds Repository, at \link{https://doi.org/10.5518/1259}.