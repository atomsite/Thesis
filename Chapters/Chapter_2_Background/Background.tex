...
In particular, we will discuss their formation, \note{growth and destruction mechanisms} in order to understand how such an object can arise in such a violent system as a CWB.
...
Of course, like all pressing questions of the late \nth{19} century, it fell to Lord Kelvin to provide a convincing -- albeit incorrect -- answer.
...
This includes stars in the \note{O-type class of the Harvard classification system, as well as some B-type stars}
\note{(B3V to B0V; \cite{zinneckerUnderstandingMassiveStar2007})}.
...
% Initial basics, star formation from first principles
...
% Virial, KH and free fall times, circumstellar disks
This collapse can be described in the simplest terms via a series of equations.
...
\noindent
where $G$ is the gravitational constant, $M_\star$ is the \note{protostellar} core mass, $R_\star$ is the radius of the core and $L_\star$ is the core luminosity.
...
Thus, the KH timescale represents the timescale for the \note{protostellar} core to radiate away its gravitational potential energy.
...
% Maximum size from radiation pressure, find citations on this
...
% Why it's more poorly understood
...
% Competitive accretion scenario and monolithic scenario 
...
% Kim 2021, duration of embedded phase
Eventually, the cloud disperses; ending the embedded phase, the massive star is now optically visible, but not after a substantial fraction of the lifetime of the star itself \parencite{kimDurationEmbeddedPhase2021}.
...
% Why binary evolution is important in this work
Binary evolution -- particularly for the fraction of massive stars that exhibit multiplicity -- is important in understanding the influence of WCd systems, as it directly influences the number of systems suitable for dust formation.
...
% Observational evidence for binary formation
...
While the binary fraction of nearby young solar mass stars is $>50\%$, there is an observed correlation between multiplicity probability and primary star mass \parencite{fuhrmannMultiplicitySolartypeStars2017}.
...
% All stars form from the collapse of giant molecular clouds (GMCs), enormous, cold clouds containing large quantities of gas.
...
% These clouds are somewhat stable, and must be perturbed in order to collapse.
% This is easier said than done, but can be induced by stellar winds from nearby stars and shock-waves from supernovae
% \parencite[Ch.~3]{bodenheimerPrinciplesStarFormation2011}.
...
% As one of these fragments condenses, it forms a \note{protostellar} core.
% This collapse can be described in the form of a series of timescales.
% Firstly, the Kelvin-Helmholtz (KH) timescale, $\tau\rms{KH}$, which is the time required for the \note{protostellar} core to radiate away its \note{gravitational potential energy}:
...
% \begin{equation}
%   \frac{d^2r}{dt^2} = - \frac{G\mass_\star}{r^2} ,
% \end{equation}
...
% In the case of a massive star, the KH timescale is significantly shorter than the free-fall timescale ($\tau\rms{KH} \ll \tau\rms{ff}$), meaning that the material at the center of the collapsing cloud begins to fuse \emph{prior} to the cessation of collapse.
% This burgeoning star begins to drive the collapsing material away through radiation pressure, blowing this material outwards, causing it to accrete and shock material within the GMC \parencite[Ch.~5]{bodenheimerPrinciplesStarFormation2011}. 
...
% Close binary systems can also form by way of fragmentation in the \note{protostellar} disk.
...
These winds have on the order of $10^{10}$ times more momentum than winds from \note{stellar mass} stars, and punch holes into the interstellar medium (ISM), forming wind-driven bubbles and champagne flows.
...
Early observations of stellar winds centred around the star \note{P Cygni}, the earliest known example of an evolved Luminous Blue Variable (LBV) star \note{\parencite{kudritzki_winds_2000}}.
...
The troughs of this spectra corresponded to a blue-shifted absorption lobe, from radiation passing through this shell, while the emission line itself corresponded to the expanding shell itself \parencite{bealsNatureWolfRayetEmission1929,lamersIntroductionStellarWinds1999}.
...
\note{In fact, winds from red dwarfs are found to be markedly denser, but the mechanism behind their winds is less understood.}
...
These dust grains \note{absorb} photons more readily than ions and atoms do through Thompson scattering, and can \note{absorb} a broad range of wavelengths due to their size.
...
\noindent
\note{
Whilst radiation pressure and Thompson scattering were considered to be too weak to drive stellar winds being observed emanating from early-type stars, radiative line driving is a significantly more efficient process.
An early-type star emits strongly in the UV, ionising the outermost regions, heavier ion species such as C, N, O and Fe group ions contain resonant lines that can readily absorb photons, with an opacity $10^6$ times greater than Thompson scattering.
The ion is excited by the photon absorption, and subsequently de-excites over a timescale of $10^{-8}$ seconds, re-emitting a photon, changing the momentum of the ion \parencite{lamersIntroductionStellarWinds1999}.
Lighter elements, coupled via Coulomb forces, are carried away with the heavier ions, resulting in an extremely efficient momentum transfer mechanism \parencite{vinkTheoryDiagnosticsHot2022}.
}

% As the ions accelerate, 



% % Line driven wind theory 
% % Line-driven winds present an alternative driving mechanism for massive stars.
% % \note{The principle is from radiation pressure, wherein a photon is \note{absorbed}, .
...
% % \note{This emission of a photon produces a recoil force on the ion, resulting in a change in momentum on the ion.}
...
% % , resulting in a change in the radial velocity, $\Delta v\rms{r}$, such that:
...
% This driving force occurs in winds enriched with elements with a large number of resonant lines, where heavier ion species from C, N, O and Fe group elements \note{absorb} the photons.
...
For instance, the C III resonance line has an energy of \SI{12.69}{\electronvolt}, so a photon requires a wavelength of \SI{977}{\angstrom} in order to be \note{absorbed} (Fig. {\ref{fig:planck-comp}}).
Additionally, photons would only be \note{absorbed} over a narrow range of frequencies.
This inhibits efficient momentum transfer from UV photons without Doppler shift.
As the outflow from the star has a distribution of radial velocities this results in a greater chance of resonance line absorption if Doppler shift is considered.
If we were to observe the outflow of a massive wind we observe a relatively low velocity component of the wind close to the star once the wind reaches a certain critical velocity \note{\parencite{cassinelliStellarWinds1979}}.
At a certain point we would observe a significant and rapid increase in the velocity of the wind, as the influence on absorption due to Doppler shift results in the wind becoming much more opaque to UV photons.
Eventually we would observe the wind reaching a terminal velocity, due to a decrease in photon flux from the inverse square law and the outflow becoming more diffuse as it spreads away from the star \note{\parencite{castor_radiation-driven_1975}}.
\note{This can be seen in Fig. \ref{fig:cak-vel}, where the velocity increases sharply at a distance of} $(R/R_\star) - 1 > 10^{-3}$ as the wind begins to rapidly accelerate away from the star as opacity increases -- with a corresponding decrease in wind density.
...
% as the force due to interaction of resonance lines with continuum photons were considered.
...
\note{These additions contributed to the development of the modified CAK (M-CAK) model, which was the standard through the 80's and 90's}.
\note{
However, there were still some limitations to this model. In particular, when simulating the denser and hotter winds of high-mass O-type and WR stars we find that M-CAK significantly underestimates their mass loss rates.
}
\note{
As computational capability increased, new models incorporating multiple scattering events using Monte Carlo methods were developed \parencite{vinkNatureBistabilityJump1999}, and modern wind models rely on calculating the radiative acceleration in the co-moving reference frame of the wind\parencite{gormaz-matamalaNewHydrodynamicSolutions2021}.
These modern methods rely less on free parameters, and are therefore more consistent \parencite{mullerConsistentSolutionVelocity2008}, and have fewer approximations such as the Sobolev approximation.
}

\note{
WR stars -- which are evolved early-type stars -- have winds that are significantly more enriched with intermediate-mass elements such as C and O through helium burning in the core.
A greater number of ions that contribute to the effect of radiative line driving resulted in inconsistent results when simulating WR stars using earlier models.
However, mass loss rates for WR stars could not be readily computed from theoretical models until more recently, with models such as the model described in \textcite{grafenerHydrodynamicModelAtmospheres2005} and the \texttt{PoWR} model described in \textcite{sanderNatureMassiveHelium2020} presenting viable alternatives to empirical models.
In the case of WC stars, it was originally assumed that C ions would be the most important species for mass loss, simulations on stars with varying metallicity suggests that the  \parencite{vinkMetallicityDependenceWolfRayet2005}.
Thus, mass loss for WC stars in the early, Fe-poor universe would be primarily influenced by intermediate mass elements such as carbon, we would therefore see WR stars with a lower mass loss rate \parencite{vinkTheoryDiagnosticsHot2022}.
This could potentially influence dust production rates in the early universe, and could present an interesting avenue for future research.
}
...
Despite being anywhere from 3 to 6 orders of magnitude brighter, the most massive stars typically only have between 1 and 2 orders of magnitude more mass to fuse.
...
\noindent
Whilst this is a fairly simplistic reduction of the stellar mass loss rate, \note{observations and} advanced models such as the \note{MESA}\footnote{Modules for Experiments in Stellar Astrophysics} project \note{\parencite{paxtonModulesExperimentsStellar2011}} show that this estimate is \note{relatively} accurate.
...
The endothermic component of the 3$\alpha$ process \note{prevents the reaction} from occurring in any reasonable quantity until core temperatures are in the order of hundreds of millions of Kelvin
\note{\parencite{harrisThermonuclearReactionRates1983}}.
...
Considering that stars previously had only been observed with narrow absorption lines, this was a particular scientific curiosity
\parencite{crowther_physical_2007}.
\note{While the mechanism behind energy production in stars was not understood, the scientific community did understand that these winds were extremely hot}.
...
\note{The thickness of these emission lines was easier to establish -- these ions were very fast moving}.
...
\note{\parencite{crowther_physical_2007}}.
% Mass loss rates, wind velocities
Whilst in the LBV phase mass loss rates can be higher, these are short but intense bursts of mass loss, rather than a continuous wind \note{\parencite{davidsonEtaCarinaeIts1997}}.
WR stars, in comparison, drive significant, continuous mass loss of the order of $10^{-5} \, \si{\solarmass\per\year}$, with wind velocities on the order of $10^3 \, \si{\kilo\metre\per\second}$. 
...
\note{\parencite{neugentWolfRayetContent2019}}.
...
\note{Whilst some WC stars are potentially capable of forming dust, these stars only produce dust in small amounts \parencite{medinaAreAllWCd2021}}.
...
The star burns neon, then oxygen, and then silicon -- the latter of which has a flurry of reaction \note{mechanisms} that produce many different elements, and burns through the entire reserves in approximately a day
...
\note{What's left behind is a neutron star; the remnant of the electron capture mechanism from the original inward dive} \note{\parencite{canalOriginNeutronStars1990}}.
...
\note{Between $10^5\,\si{K}$ and $10^9\,\si{K}$ the following approximation is valid}:
...
For \note{our dust model}, this threshold temperature was defined as \SI{14000}{\kelvin}\footnote{\note{Our reasoning for this value is detailed in Section \ref{sec:ddustevol}.}}.
...
\note{The species of interstellar dust studied in this thesis are composed of carbon}.
...
\note{\parencite{ladaEmbeddedClustersMolecular2003}}.
...
\note{\parencite{heyerMolecularCloudsMilky2015}}.
...
\note{\parencite{williamsProtoplanetaryDisksTheir2011}}.
...
These photons can leave the medium, and in dense mediums this can occur over multiple absorption/re-emission events.
...
At temperatures from 10 to $100 \, \si{\kelvin}$ atomic excitation is extremely unlikely, other than heating events from absorption of cosmic rays.
...
\note{
Between $100 \,\si{K}$ and $1000 \, \si{K}$, radiative cooling is dominated by the hydrogen forbidden line transition.
...
Gas at this temperature is relatively transparent to radio waves, and as such the \SI{21}{cm} line can steadily remove energy through cooling.
...
\noindent
These transitions release significantly more energy than the comparable, \note{lower temperature \SI{21}{\centi\metre} transition, and as such are a more efficient way of removing energy from metal enriched winds, explaining the increase in cooling rate in the WC abundance wind in Fig. \ref{fig:wcsolcooling}}.
...
In order to explore the contribution of cooling to the structure of the WCR, we must do what many astrophysicists despise the most -- describe something mathematically.
...
\begin{figure}[h]
  ...
  \caption[A comparison of the influence of radiative cooling on a CWB system]{A comparison of the influence of radiative cooling on the structure of a CWB system. A system with a momentum ratio of $\eta = 0.02$ is simulated with an increasing orbital separation, such that $\chi$ increases. As can be seen the WCR becomes adiabatic and smooth the more $\chi\rms{WR}$ increases.}
...
Collisional excitation and absorption of photons can stochastically heat dust grains.
...
\note{
  However, as population III stars are theorised to be significantly higher mass the proportion of stars that underwent either a WR or analogous phase would be higher. As we can observe that there is no AGB mass loss at high redshifts, we can assume AGB stars would not contribute to dust production in the low-metallicity early universe \parencite{morganDustFormationEarly2003}.
  As such, WCd stars would have a much greater impact on the formation of dust in the early universe and would be highly influential on the deposition of carbon and dust into early galaxies \parencite{soulainSmokeWindDust2022a}.
  Whilst there is limited analysis of the impact of these stars in the early universe, this would present an interesting avenue of research in the future.
}
...
\begin{threeparttable}[h]
  ...
  \end{tabular}
  \begin{tablenotes}
    \item[1] \note{\textcite{hendrix_pinwheels_2016};}
    \item[2] \note{\textcite{lauRevisitingImpactDust2020};}
    \item[3] \note{\textcite{harriesThreedimensionalDustRadiativetransfer2004};}
    \item[4] \note{\textcite{soulainSPHEREViewWolfRayet2018};}
    \item[5] \note{\textcite{williams_dust_1990};}
    \item[6] \note{Calculated}
  \end{tablenotes}
...
\begin{threeparttable}[h]
  ...
  \end{tabular}
  \begin{tablenotes}
    \item[1] \note{\textcite{rossloweSpatialDistributionGalactic2015};}
    \item[2] \note{\textcite{hendrix_pinwheels_2016};}
    \item[3] \note{\textcite{lamberts_impact_2012};}
    \item[4] \note{\textcite{soulainSPHEREViewWolfRayet2018};}
    \item[5] \note{\textcite{thomasOrbitStellarMasses2021};}
    \item[6] \note{Calculated}
  \end{tablenotes}
...
\begin{figure}[h]
  ...
  \caption[JWST image of WR140]{\note{JWST MIRI image of the WCd system WR140, asymmetric dusty ``rings'' are observed, indicating that the system is a episodic WCd system \parencite{lauNestedDustShells2022}.}}
  \label{fig:jwst-wr140}
\end{figure}

% Historical observation
\note{WR140 (Fig. \ref{fig:jwst-wr140})} is significant in that it is the first system observed with episodic WCd properties.
