...
\turbo{This whole section needs rewriting}
...
Firstly, the Kelvin-Helmholtz (KH) timescale, $\tau\rms{KH}$, which is the time required for the proto-stellar core to radiate away its \note{gravitational potential energy}:
...
\noindent
Whilst the reaction is direct and efficient, due to its high energy production per nucleon (Table \ref{tab:reactionrates}); the reaction rate has a poor temperature dependence of $\epsilon \propto T^{3.5}$ due to the small reaction cross-section.
...
\noindent
The CNO I cycle -- which was also discovered by Bethe -- has a markedly higher temperature dependence on the reaction rate, $\epsilon \propto 10^{12}$
(\cite[Ch.~10]{wongIntroductoryNuclearPhysics1998,betheEnergyProductionStars1939,lamersUnderstandingStellarEvolution2017}; also see Fig. \ref{fig:fusionrates}).
The CNO cycle is dominant at internal temperatures $>\SI{15.9}{\mega\kelvin}$, corresponding to a stellar mass of $\gtrsim \SI{1.3}{\solarmass}$ \parencite[121]{salarisEvolutionStarsStellar2005}.
...
This results in a convective core surrounded by a radiative envelope, and is the driving force behind the observed luminosities of high-mass stars as they convert hydrogen to helium at an immense rate \parencite[Ch.~5]{salarisEvolutionStarsStellar2005}.
...
A photon with an energy equal to the excitation energy of an emission line of an ion in the wind is \note{absorbed}, exciting the ion.
...
Whilst some WC stars are potentially capable of forming dust, this is predicted to be rare, and these stars only produce dust in small amounts \parencite{medinaAreAllWCd2021}.
...
What's left behind is a neutron star: the remnant of the electron capture mechanism from the original inward dive \parencite[Ch.~13]{longairHighEnergyAstrophysics2011}.
...
\noindent
These transitions release significantly more energy than the comparable, lower temperature \SI{21}{\centi\metre} transition, and as such are a more efficient way of removing energy from metal enriched winds, explaining the increase in cooling rate in the WC abundance wind in Fig. \ref{fig:wcsolcooling}
\parencite[Ch.~5]{dysonPhysicsInterstellarMedium2021}.
\turbo{Change citation}
