...
\note{Hydrodynamical simulations are an extremely processing intensive, but highly parallelisable problem.}
A hydrodynamical problem can be divided into a series of smaller problems and solved independently for a common timestep.
These numerical grids must be checked for concurrency at the interfaces between grids, which \note{introduces some redundancy and reduces overall parallel efficiency}, but on the whole is significantly faster than \note{with single-threaded execution}.
This subdivision can be performed through striping along one of the dimensional axes, with stripes distributed to a specific processor (as performed by \mg{} and \footlink{\texttt{VH1}}{https://wonka.physics.ncsu.edu/pub/VH-1/}).
Another method for parallelism is reducing the entire problem to a series of blocks \note{stored in an octree or similar datastructure} that are \note{subsequently} solved in parallel (performed by \athena; \cite{stoneAthenaAdaptiveMesh2020}). 
...
For instance, the laws of physics have remained consistent over the last 13.8 billion years, and as such simulations have been performed of the early universe.
...
\begin{itemize}
  ...
  \item The structure can also be obscured by the parent stars, which are \note{high-mass stars with luminosities $>\SI{1e5}{\solarluminosity}$}.
...
\noindent
Observation of the large-scale dusty structure of the WCR is possible with current telescopes, and the surrounding dust cloud (and pinwheel structure in the case of persistent WCd systems) has been observed in multiple WCd systems \note{\parencite{soulainSPHEREViewWolfRayet2018,callinghamAnisotropicWindsWolf2019,lauNestedDustShells2022}}.
\note{This feature, while impressive and useful for discerning whether a system is episodic or persistently dust forming, does not undergo significant dust formation, which instead occurs a short distance from the apex of the WCR \parencite{harriesThreedimensionalDustRadiativetransfer2004}}.

\note{Observation of the apex and dust formation zone of the WCR is markedly more difficult, as it is an extremely compact feature compared to the outflowing spiral or ring structure.}
\note{In the case of WR140, with a periastron of \SI{1.53}{AU} and a distance from Earth of \SI{1720}{pc}, a minimum angular resolution of \SI{0.44}{\milli\arcsecond} is required to resolve the stars separately, in order to observe the apex of the wind collision region we would need to be significantly lower than this, perhaps by one or two orders of magnitude.}
\note{For comparison, the James Webb Space Telescopes NIRCam instrument has an angular resolution of \SI{70}{\milli\arcsecond} at a wavelength of \SI{2}{\micro\metre}, which is insufficient to resolve the innermost region of the WCR.}
\note{Interferometers -- such as the VLTI GRAVITY instrument -- offer an increased resolution at similar wavelengths, but are still insufficient (GRAVITY, for example, has an angular resolution of \SI{3}{\milli\arcsecond} in the K band).}
\note{Additionally, the brightness of the stars is very high, with a combined luminosity on the order of \SI{2e6}{\solarluminosity} \parencite{williamsResults2009Campaign2011}.}
\note{Whilst this emission is primarily in the ultraviolet, this would make the innermost region of the WCR more difficult to observe and would potentially necessitate the use of a coronagraph in a similar method to the direct detection of exoplanets.}
...
Radiative transfer models can also be used to model the dust production rate of these systems\footnote{\note{However, this was not feasible for this project, due to time limitations.}}.
...
\note{In order for the WCR to evolve correctly, the apex of the region must be properly resolved from either of stars and must also have a sufficiently high cell resolution to resolve the two shock regions separately. However, as the WCR apex is the smallest feature of the system, the separation resolution should be minimised without violating these conditions. In the case of a WR+OB CWB system the resolution must also be higher as the significantly stronger WR wind situates the WCR apex closer to the OB star.}
...
All of these factors enact a significant computational cost, requiring numerous cores to effectively simulate the systems.
...
Initially this project intended to use the \mg{} hydrodynamical code \parencite{falleSelfsimilarJets1991}, as CWB systems had already been simulated, and the resultant code being readily available within the department.
\mg{} also had a number of features that were useful for this project, such as MPI parallelism and adaptive mesh refinement.
It was initially estimated that it would take little more than a year to implement the dust and cooling models; with large-scale simulations on the ARC3 and ARC4 computers being commenced midway through the second year of research.
This was unfortunately not the case.

% why we didn't use it in the end

It was found that our dust model was unreliable when implemented in \mg.
The dust model either produced dust rates measurable in grams per year, or the simulation rapidly converted all gas into dust.
Attempts to implement the dust model through modification of the conserved variables or through a rate-based source function were made, with multiple different implementation attempts.
None of these were reliable, resulting in a large amount of work being discarded.
Using strict constraints to prevent rigorous dust production resulted in strange looking systems that did not agree with observational data.

In addition to incompatibility with the dust model, the wind mapping implementation resulted in additional issues.
In order to get the required separation resolution for systems with close orbits, a very high resolution before mesh refinement was required, massively increasing memory usage and removing the benefits of mesh refinement.
Using a source function to simulate wind was considered, which would have allowed for refined cells to map wind, but this would produce winds with unrealistically high temperatures, on the order of \SI{1e7}{K}.

% While being very extensible in terms of being able to implement a problem generator fairly easily, low-level manipulation of the \mg{} source code was found to be extremely difficult due to limited documentation and a complex, linked-list grid structure.
...
In the end, the decision to switch from \mg{} to the then-new \athena{} hydrodynamical code was made in mid-2020.
...
The \footlink{\athena{}}{https://github.com/PrincetonUniversity/athena} \note{hydrodynamical code was found to be a more suitable fit for this project}.
...
Multiple time-integration and spatial reconstruction methods are implemented into \athena{}.
The time-integration method can vary from a computationally simple \nth{2} order van Leer \parencite{vanleerUltimateConservativeDifference1979} method to strong stability preserving methods \parencite{ruuthHighOrderStrongStabilityPreservingRungeKutta2005} to super time-stepping Runge-Kutta-Legendre methods \parencite{meyerStabilizedRungeKuttaLegendreMethod2014}.
...
For this project either the \nth{3} order accurate strong stability preserving Runge-Kutta method (\texttt{rk3}) or the \nth{4} order accurate, five-stage, 3 register, SSPRK method (\texttt{ssprk5\_4}) were utilised -- depending on the instability of the simulation.
...
A problem previously discussed with modelling CWB systems is that the apex of the WCR is small relative to the system itself.
...
The results are detailed in Table \ref{tab:amr-comparison}, where we find a significant speedup and reduction in cell count and memory usage.
...
Unfortunately, there is a known issue with \footlink{\athena{}}{https://github.com/PrincetonUniversity/athena/issues/365} which prevents the use of AMR with advected scalars enabled.
...
% Radiation processes
It is also assumed that all energy radiated leaves the simulation, with no subsequent re-absorption.
...
Whilst re-absorption and subsequent re-emission would significantly increase the complexity of the simulation, as the effects of radiative transfer would have to be calculated.

% Short coverage on dust model, this is explained further in a alter section
Finally, a number of assumptions are made for the dust model.
...
\note{We would expect to address this in future, multi-fluid versions of this model, however.}
\note{Additionally, we assume that grain growth begins to occur below temperatures close to \SI{1e4}{K}, which is the typical floor temperature of CWB simulations \parencite{pittard_3d_2009,parkinInvestigatingXrayEmission2011}.}
\note{Meanwhile, dust destruction processes do not become significant in the model until the gas temperature exceeds \SI{1e6}{K}, when a micron-sized grain has a lifespan of approximately \SI{1e7}{\year} in a flow of density \SI{1}{g.cm^{-3}}.}
\note{We also adopt a 10\% efficiency factor for dust grain growth, in low temperature environments with uncharged grains this can be as high as 100\% according to \textcite{spitzerPhysicalProcessesInterstellar2008}, but is much lower in the expected conditions of a WCd system \parencite{leitch-devlinStickingCoefficientsAtoms1985}.}
\note{The assumptions for the dust model are discussed in further detail in Section \ref{sec:dustimplementation}.}
...
\note{In Section \ref{sec:numerical-purpose} we discussed the minimum resolution limits of the WCR, and how this effects the resolution of the simulation as a whole.}
\note{In short: the finest cell resolution must be sufficiently high as to ensure that the shocks within the WCR are resolved from each other and from the parent stars.}
\note{Radiative cooling also increases the resolution requirement, as the post-shock region can be significantly more compressed.}
\note{The resolution must also be kept as low as possible so as not to increase the computational cost of the simulation.}
\note{There is limited discussion on separation resolution in available literature, and would vary depending on the wind properties of the system, in particular $\eta$.}
\note{\textcite{pittard_3d_2009} uses a separation resolution of $\sim 100$ cells for a non-adiabatic OB+OB binary system with a close orbit. This was used as a starting point to determine an appropriate separation resolution.}
\note{Testing was conducted by running simulations with a progressively lower simulation and finding whether mixed winds were present in the remap zones, which would indicate that the WCR was not resolved form the star.}
\note{We found a separation resolution of 100 cells was suitable for WR98a and WR140, while simulations with a separation resolution of 30 cells would not produce consistent results.}
\note{As such our simulations in \textcite{eatsonExplorationDustGrain2022} used a separation resolution of $\sim{120}$ fine cells including remap zones, while simulations of WR140 in \textcite{eatsonExploringDustGrowth2022} utilised a separation resolution of $\sim{100}$ fine cells including remap zones. The reduction in the latter was for the sake of limited simulation time, but did not result in winds impinging on the remap zones.}
\note{Remap zones were specified to be 12 fine cells in radius. 3 fine cells was found to be the minimum value producing radial outflows, but increased resolution was needed to produce spherical outflows with no fluctuation.}
...
\note{This was done as re-interaction would significantly increase the computational cost for each time-step.}
Due to this, scattering, re-absorption and radiative transfer are not simulated at all.
...
For the case of larger and multi-parameter lookup tables this method would only improve in performance, and is a good example of general optimisation in a numerics programme.

% Method of integration chosen
In order to integrate the energy loss rate to determine the exact amount of energy lost within a timestep, an integration method needs to be chosen.
...
When debating whether to include a feature in a numerical simulation, we must weigh up the computational cost versus necessity of inclusion.
...
Performing an indefinite integral inside a hydrodynamical code is not ideal, and must be either constrained or simplified in such a way that it can be made performant.
...
The first -- and most na\"ive -- method of determining $h_e$ considered was by calculating $h_e$ for each cell at each cooling step.
...
Table \ref{tab:electron-speedup} shows the results of benchmarking attempts on all the methods tested.
...
In order to simulate energy loss due to radiation in \athena{}, the conserved variable array is adjusted to remove energy from a specific cell.
...
Improved models in the future could utilise an adaptive RK method, similar to the numerical integrator in \athena{}, though the implementation attempted had some numerical stability and execution time issues, and would require significant optimisation.
...
% Broad featureset
Currently, the \bidmas{} model supports grain advection, growth and destruction, as well as the dust cooling model discussed in Section \ref{sec:dustcoolingmodel}.
...
Whilst much of the work in this thesis was accomplished on a slightly earlier build of this model, a more advanced, cleaned up version of the model has been \footlink{written}{https://github.com/atomsite/athena}.
...
% Testing
\note{Initial testing was conducted by first simulating the evolution of dust in a 0-D scenario, where a region with a set initial density was left to evolve for an in-simulation year.}
\note{These evolution tests are seen in Fig. \ref{fig:initialtesting}, where dust yields were strongly dependent on the gas density and time elapsed, producing yields exceeding 1\% in the case of a region with $\rho\rms{g} = \SI{1e-14}{g.cm^{-3}}$.}
% Limitations to testing
\note{This initial testing phase was brief, and mainly used to determine whether the grain growth equations would produce sensible results or affect the stability of a hydrodynamical simulation.}
\note{This is mainly because this method does not account for expansion of the post-shock gas, nor does it account for the initial rapid cooling phase; these two limitations combined result in a significant over-estimation of dust production.}
\note{Instead, simple 3-D simulations with a similar elapsed time of a year and a limited numerical domain were conducted before running full-scale simulations.}
\note{These simulations typically had a run time of a day, and as such were suitable for rapid testing.}
\note{These simple 3-D tests, and initial parameter searches are discussed in more detail in Section \ref{sec:injection}.}

\begin{figure}
  \centering
  \includegraphics{assets/initial-dust-test.pdf}
  \caption[\note{Initial testing of the dust model}]{\note{Initial, ``0-D'' testing of the dust model, where a gas of a specific density and a small amount of initial dust ($z_i = 10^{-10}$, $a_i = \SI{0.01}{\micro\metre}$) was left to evolve at a temperature of \SI{1e4}{K}. This testing does not account for cooling and expansion, and thus over-estimates the amount of dust produced at these conditions.}}
  \label{fig:initialtesting}
\end{figure}
...
\begin{figure}[p]
  \centering
  \includegraphics{assets/a_z_tweaking/a_evol.pdf}
  \caption[\note{$a$ evolution with varying $z_i$}]{\note{A comparison of the average value for $a$ in the WCR over the course of a simulation with WR98a properties. $z_i$ and $a_i$ are varied for each simulation. We find that grain growth occurs more rapidly in simulations with a smaller initial grain radius, and that changing the value of $z_i$ does not change the final average grain radius significantly.}}
  \label{fig:a_evol}
\end{figure}
...
\note{These simulations were also conducted to determine the stability of the grain growth model when incorporated into \athena{}.}
...
\note{Each simulation had a coarse cell resolution of $320 \times 320 \times 40$ XYZ cells with 4 levels, for an effective resolution of $2560\times 2560 \times 320$ XYZ cells.}
\note{Whilst this is the same coarse resolution as the simulations conducted \textcite{eatsonExplorationDustGrain2022} (Ch. \ref{chap:parameterspace}), the number of levels is lower, resulting in $1/512^{\text{th}}$ of the effective resolution of the full-scale simulations outside of the wind collision region.}
\note{The simulation domain was also smaller, with a size of $256 \,\si{AU} \times 256 \,\si{AU} \times 32 \,\si{AU}$, which allows for faster advection of the simulation.}
These simulations were run over the course of a week on the ARC4 HPC cluster \note{using 64} cores.
...
As can be seen in Fig. \ref{fig:azweak_img}, we find that decreasing $a_i$ creates a much more defined WCR, at the cost of increasing dust outside the WCR.
...
As we do not observe significant quantities of dust being produced outside the WCR, we prioritised a small value of $z_i$, as long as it does not significantly influence dust yields.
...
\note{We also observed a similar lack of change in $a$ when $z_i$ is varied (Fig. \ref{fig:a_evol}).}
\note{$a$ is also noted to not increase significantly overall, even in the case of small initial grain sizes.}
\note{For instance, in the case of simulations with an initial grain size of \SI{1e-2}{\micro\metre}, we observe a rapid initial grain size increase, which quickly slows to a final average size of \SI{3e-2}{\micro\metre}, consistent with \textcite{zubkoPhysicalModelDust1998a}.}
We can therefore discern from these previous two results that $z_i$ should be kept to a fairly small value.


The parameter $z_i$ was found to not affect the dust production within the WCR significantly.
...
After this preliminary parameter space exploration and additional stability testing, an $a_i$ of \SI{50}{\angstrom} and a $z_i$ value of $10^{-8}$ were utilised.
...
\note{\athena{} was also modified to check each cell in the simulation for unrealistic dust conditions, such as dust density increasing beyond the total amount of gas in a cell, and grain sizes beyond \SI{1}{\micro\metre} (the scalar primitive value limit). If a cell is detected violating these conditions the simulation is ended and the user is alerted.}
...
\note{This is an inherent limitation of being unable to simulate grain-grain interactions in the current iteration of the \bidmas{} model.}
% In the worst case, the dust number density may not be correctly calculated, which can affect the rate of cooling.
% Multiple grain sizes and coupling
\note{Another limitation of the advected scalar model worth considering is that there} is no variation in grain size within each numerical cell.
\note{However, the simulations we conducted in \textcite{eatsonExplorationDustGrain2022} (see Chapter \ref{ch:wr140}) show that grain size does not vary significantly within the WCR itself.}
% Limitations due to this
\note{As an advected scalar relates to properties that co-move with the fluid, dust grains in this model are therefore simulated as perfectly co-moving.}
whilst other models assume that dust grains are \note{instead} coupled through a drag force, it was found that the dust was effectively coupled to the wind as if it was co-moving (Section \ref{sec:hendrixmodel}; \note{\cite{hendrix_pinwheels_2016}}).
\note{It is important to note that this may prevent significant dust mixing in the immediate post-shock region due to the increased inertia of the dust grains compared to the gas, which would be important to consider with a more advanced chemical model.}
% Grain-grain collision
As a single grain size for each cell is assumed, dynamics between grains of different sizes, such as grain agglomeration \note{and shattering} cannot be accurately simulated.
% Fixes
...
All grains in the simulation are assumed to be circular, \note{and thus have} a volume, $V\rms{gr}$, of $4/3 \pi a^3$ and a mass, $m\rms{gr} = V\rms{gr} \rho\rms{gr}$, where $\rho\rms{gr}$ is the grain bulk density.
As we assume all dust grains in the simulation are composed of amorphous carbon, we \note{adopt a value of} $\rho\rms{gr} = \SI{3.0}{g.cm^{-3}}$ \parencite{bhattaraiEvolutionAmorphousCarbon2018}.

% Gas temperature, restate floor temperature of 10,000K
\note{Other important assumptions for this model that must be justified involve the high temperature of formation and dust growth, as well as the lack of immediate grain destruction through UV photodissociation and heating.}

% Formation pathways at high temperatures
\note{In the case of CWB systems, a high temperature dust formation pathway occurs at temperatures on the order of \SI{4e3}{K} and gas number densities exceeding \SI{1e10}{cm^{-3}}.}
\note{This process involves the formation of carbon chains, which form into rings and then intermediate fullerene caged species \parencite{cherchneff_dust_2010}.}
\note{Such conditions are observed in even highly imbalanced WR140 systems at periastron, such as WR140 \parencite{williams_orbitally_2009,williamsConditionsWR1402021}.}
% Appropriate to simulate cooling, though a limitation
\note{Whilst we do not simulate temperatures below \SI{1e4}{K} in our simulations for the sake of stability, these observations indicate that cooling below this point does occur.}
% UV shielding
\note{The lack of destruction through UV radiation emanating from the parent stars is an important assumption for this model.}
\note{Most theories for dust formation in WCRs involve UV shielding from high density clumps in which the dust forms, as well as occultation from the compressed, post-shock winds and other clumps \parencite{tuthill_prototype_2008,williams_orbitally_2009,lauRevisitingImpactDust2020}.}
% Harries, optical depth, significant drop in temperature
\note{In addition, \textcite{harriesThreedimensionalDustRadiativetransfer2004} provides a quantitative calculation of this effect, they find that when there is significant dust mass and optical depth in the first spiral revolution, shielding can prevent strong UV heating, with the second turn showing a significant drop in temperature as well, dust formation is also a net positive, despite being above the expected sublimation temperature.}
\note{In addition to shielding through the post-shock wind and clumps, the decrease in flux $\propto r^{-2}$ with a slower reduction in post-shock wind density would result in an increasingly more favourable environment for dust growth in the WCR.}
% Formation pathways at low temperatures
\note{It is also important to note that an alternative, lower temperature path exists, wherein a catalysed reaction with hydrogen can occur, which could further increase dust production rates \parencite{herbstComplexOrganicInterstellar2009}.}
\note{This is not simulated, but future work could explore this alternate formation pathway.}
...
The gas is assumed to be optically thin to infrared, and the radiative losses are therefore removed from the simulation without re-absorption.
...
\noindent
\note{where $C$ is the wind ``colour'', and $X,Y,Z$ are the individual wind hydrogen, helium and metal mass fractions, respectively
\parencite{mihalasStellarAtmospheres1978,pittard_x-ray_1999}.}
...
As photoionisation maintains the wind temperature of these systems at approximately \SI{1e4}{K}, the floor temperature of the simulations was set to this temperature.
This is a typical floor temperature value of CWB simulations \parencite{pittard_3d_2009,parkinInvestigatingXrayEmission2011} due to this reason. The simulation is also more numerically stable with a floor temperature of \SI{1e4}{K} when radiative cooling is introduced.
For temperatures above $10^6 \, \si{K}$, dust destruction occurs, while at temperatures below \SI{1.4e4}{K} \note{-- slightly above the floor temperature --} dust growth occurs instead.
...
\noindent
Throughout this thesis we use a grain sticking factor of 0.1, \note{which is a reasonable estimate based on research by \textcite{leitch-devlinStickingCoefficientsAtoms1985}}.
...
Multiple grain size distributions could also be modelled similarly to the proposed multi-scalar model, but with the kinematics of the dust grains being simulated separately.
% Mixing factors
