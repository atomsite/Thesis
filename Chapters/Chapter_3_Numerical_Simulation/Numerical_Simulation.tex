...
Cooling is crucial to our 
in a purely adiabatic case, gas temperature in the post-shock region can 

% Why even include cooling
Additionally, adiabatic flows behave differently without a method of losing energy, density, pressure and post-shock velocity are constrained, which can drastically effect the evolution of the system, and prevent significant cooling down to the 
% Need to go over my old winds work!
In the case of strong, adiabatic shocks, the downstream gas parameters of the system are:
...
in a radiative shock behaving isothermally (where the temperature change, $\Delta T$ throughout the entire lifespan of the fluid is equal to zero), the final density, $\rho_f$ can be approximated to:
...
Thus, instead of calculating the emissivity of the plasma for the current density, temperature and abundance, 
...
% Speed versus accuracy comparison
...
\subsubsection{Exact cooling method}

%//TODO write this, you will have to ask Julian about this

An exact integration scheme for radiative cooling described in \cite{townsendExactIntegrationScheme2009} was considered to simulate cooling in Athena++. 

% Explanation of method, complex bit

% Why would this be good
This scheme is one of the rare example of a numerical method that is both accurate \textit{and} fast, taking approximately the same time as a second order explicit method, whilst also being perfectly accurate even in highly radiative hypersonic flows.
...
Second, and perhaps most importantly, only plasma cooling would have been possible, dust cooling could not be modelled at the same time, resulting in two separate cooling stages, as both mechanisms are highly temperature dependent, this was not acceptable.
