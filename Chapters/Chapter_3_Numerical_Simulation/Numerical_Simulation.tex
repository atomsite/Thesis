\chapter{Methodology \& Numerical Simulations}
\label{ch:numsim}
...
The second method of solving the $h_e$ integral is by using a series of approximations described by \cite{dwek_infrared_1981} where the 

\begin{equation}
  \begin{alignedat}{3}
    h(x^*) & = 1 ,                && ~~ x^* > 4.5, \\
           & = 0.37{x^*}^{0.62} , && ~~ x^* > 1.5 , \\
           & = 0.27{x^*}^{0.27} , && ~~ \text{otherwise,}
  \end{alignedat}
\end{equation}

where $x^* = 2.71 \times 10^8 a^{2/3}(\si{\micro\metre}) /T$.
Whilst this is less accurate, especially in the region where one case ends and the other begins, this is significantly faster.
...
% Further optimisations, electron density lookups
