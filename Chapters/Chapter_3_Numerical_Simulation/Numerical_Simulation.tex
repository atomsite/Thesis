...
In particular: how they work and why they are being utilised, as well the development and implementation of a CWB model inside the \athena{} hydrodynamical code.
...
In a one-dimensional adiabatic case, with a fluid of density, $\rho$, a velocity of $u$, a fluid pressure, $P$, and a total energy, $E$, the Euler equations take the form:
...
The Riemann problem is a composed of a series of conserved variables on each side of a discontinuity (see Fig. \ref{fig:riemann}).
...
Piecewise linear methods linearly interpolate fluxes between cells to reconstruct the cell interfaces, this is generally considered to be significantly more versatile than a simple piecewise constant method \parencite{vanleerUltimateConservativeDifference1979}.
...
This subdivision can be performed through striping along one of the dimensional axes, with stripes distributed to a specific processor (as performed by \mg{} and \texttt{VH1} \turbo{Julian, is there a specific citation for VH1?}).
...
For instance, the laws of physics have remained consistent over the last 13.8 billion years\footnote{With some earlier exceptions, see cosmological inflation.}, and as such simulations have been performed of the early universe.
...
This could also produce artefacts at level transitions, however, while also producing wind temperatures in the order of \SI{1e7}{K}.
...
As there was ultimately no time to address this bug, the decision was made to persist with using static mesh refinement (SMR) for the second papers work, despite a version of the code already being written with AMR in mind.
...
As such, SMR was found to be suitable for our simulations, as a fix was not available for AMR from the \athena{} developers before publication.
...
Conversely, we assume that dust destruction processes are not significant until the gas temperature is above $10^6 \, \si{K}$.
...
\begin{subequations}
  \begin{align}
    \rho\rms{R} &= \frac{\dot{\text{M}}}{4 \pi r^2 v_\infty}, \\
    p\rms{R}    &= \rho_R v_\infty^*, \\
    E\rms{R}    &= \frac{P_R}{\gamma - 1} + \frac{1}{2} \rho_R v_\infty^2,
  \end{align}
\end{subequations}
...
\noindent
in a co-moving reference frame where $a$ is the upstream side and $b$ is the downstream, post-shock side.
...
A typical lookup table in this project utilises logarithmically spaced temperature bins from $10^4\,\si{\kelvin}$ to $10^9\,\si{\kelvin}$, with 101 bins in total, if the calculated temperature is between bins a linear interpolation step is used to improve the accuracy of the emissivity solution.
...
Other optimisations relied on replacing a na\"ive linear search with an indexing method that relied on the logarithmic spacing of the temperature bins.
...
Other methods of refining the emissivity value were also considered, such as fitting a local curve to the data or using a spline-based interpolation step instead of a linear step, however these were only marginally more accurate, while significantly increasing calculation time. 
...
First, this method would not have been able to accurately model mixed wind situations, hampering its usage cooling winds with different abundances.
...
\begin{figure}[ht]
  ...
  \caption[Plot of $\mathcal{K}(x^*,z)$ for discrete values of $z$]{Plot of $\mathcal{K}(x^*,z)$ with values of $z$ between 0.001 and 1, we see that the plot are well constrained between $z=10^{-3}$ and $z=10^2$. Therefore, the equation can be calculated in the form of a definite integral.}
...
Through testing it was found that 400 bins was the minimum amount required to reliably calculate $h_e$, which would result in the integral taking approximately 90\% of the overall execution time of a typical cooling cycle.
...
Additionally, this model was designed be extensible; while grain destruction and accretion mechanisms that were the most influential were implemented first, a number of additional mechanisms could be implemented later for improved accuracy.
The Binary Interaction Dust Model with Accretion and Sputtering (\bidmas)\footnote{Any good thesis (though this one, again, is of debatable quality) has an incredibly laboured acronym!} model is the result of this work, and only a few dust evolution mechanisms were implemented due to time restrictions, it is a good first step towards modelling dust growth in WCd systems.
...
The other parameter, $z_i$, was found to not effect dust production within the WCR significantly.
Therefore, we could reduce the effective initial parameter space of dust in the wind to a single value, $a_i$, which was found to operate best when kept at realistic values.
...
% Number density
We assume that the number density, $n\rms{d}$, is constant throughout the simulation, which is calculated based on the initial grain radius and the initial dust-to-gas mass ratio.
% Limitations due to this
We therefore assume that the net rate of grain shattering and grain agglomeration is zero.
...
We also assume that there is little variation between grain size within the numerical cell.
% Limitations due to this
...
This would be more difficult to implement than a single model but would be able to accurately simulategrain-grain collision, and better calculate dust growth and destruction rates.
...
Multiple grain size distributions could also be modelled in a similar way to the proposed multi-scalar model.
But with the kinematics of the dust grains being simulated separately.
% Mixing factors
